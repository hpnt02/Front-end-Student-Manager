...
function QLGV() {
  const columns = [
    ...
    },
    {
      ...
      width: 200,
      renderCell: (params) => (
        <div style={{ display: 'flex',cursor: 'pointer', alignItems: 'center', justifyContent:'center' }}  onClick={() => handleLGDClick(params.row)}>
          <Edit
            style={{  color: 'blue' }} 
...
const [records, setRecords] = useState(resultArray)
 function handleFilter(event) {
  ...
  const newData = resultArray.filter(row => {
    ...
    return (
      tenGiaoVien.includes(searchValue) ||
      maGV.includes(searchValue)
    );
  });
...
useEffect(() => {      
        ...
        <div className={cx('wrapperr')}>
         <div className={cx('innerr')}>
                <div className={cx('bgr-content')}>
                 ...
                 <div className={cx('search')}><input type="text" onChange={handleFilter} placeholder="Nhập mã giáo viên hoặc tên giáo viên"/></div> 
                 <div className={cx('button-create')}>
                 {window.location.pathname === '/quan-ly-giao-vien' ? (
     <>
              <Button type="text" primary to={config.routesAdmin.tmgv}>Thêm mới giáo viên</Button>  
              <Button type="text" disabled={selectedIDs.length === 0} primary onClick={handleClickCancel}>Xóa giáo viên</Button>   
              </> 
                 ):""}
  ...
  {onCancel ? 
                  ...
                  <DeleteGiaoVien onCancel={handleUpdateDiemCancel} giaovientId={selectedIDs}/>:""
                }
