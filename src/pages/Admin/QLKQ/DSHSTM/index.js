...
function DSHSTM() {
  const location = useLocation();
    ...
    const newDiem = diem.filter((state)=> state.MonHoc === MonHoc._id && state.NamHoc === namHoc && state.HocKy === hocKy)
    ...
    const handleExport = () => {
      ...
      const title2=[["STT","Mã học sinh","Họ và tên học sinh",'Điểm hệ số 1',"","","", 'Điểm hệ số 2',"","","","Điểm TBKT", "Điểm CK", "Điểm TB" ,"Ghi chú"]]
      const headers = [["", "", "",'Điểm L1', 'Điểm L2', 'Điểm L3', 'Điểm QT', 'Điểm L1', 'Điểm L2', 'Điểm L3', 'Điểm QT',"", "", "", ""]];
      const rows = resultArray.map(item => [item.id, item.MaHocSinh, item.TenHocSinh, 
            ...
            <div className={cx('innerr')}>
              ...
              <Button  Remove onClick={removeFile}>
                Remove file
                              ...
                              <div>
                            <PDFDownloadLink document={<BangDiemTheoMon hs={resultArray} infor={infor}/>} fileName="BangDiemHocSinh">
     <Button primary>Print PDF</Button>
         </PDFDownloadLink> 
         {/* <BangDiemTheoMon hs={resultArray} infor={infor}/> */}
                              <Button primary onClick={handleExport} className={cx('export-excel')}>Xuất Excel</Button>
                            {user === '1' ?  <Button primary to={config.routesAdmin.qlkhht}>Quay lại</Button> :
                            
                            <Button primary to={config.routesAdmin.dslgd}>Quay lại</Button>
                            }
                        ...
                        <TableBangDiemTheoMon row={resultArray} onClick={handleIDHocSinhChange}/>
                            {
                              onCancel ? 
                              <UpdateDiem MonHocID={MonHoc._id} studentId={selectedHocSinh} NamHocID={selectedNamHoc} HocKyID={selectedHocKy} onCancel={handleUpdateDiemCancel}/>:""
                            }                                                                                                      
                    </div>
