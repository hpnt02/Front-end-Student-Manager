...
import TableTest from '~/components/TableBangDiem';
...
function BDHS() {
    ...
    namhoctuongung.forEach((nh) => {
        ...
        hockytuongung.forEach((hk) => {
            ...
            const filteredItems = resultArray.filter(
                (item) => item.NamHoc === nh._id && item.HocKy === hk._id && item.DTB >= 5,
            );
            ...
            if (filteredItems.length > 0) {
                ...
                DiemTichLuy.push({
                    ...
                    XepLoaiHocLuc:
                        DiemTrungBinhTichLuy >= 9
                            ? 'Xuất sắc'
                            : DiemTrungBinhTichLuy >= 8
                            ? ' Giỏi '
                            : DiemTrungBinhTichLuy >= 7
                            ? 'Khá'
                            : DiemTrungBinhTichLuy >= 5
                            ? 'Trung Bình'
                            : 'Yếu',
                });
            }
                ...
                <div className={cx('bgr-content')}>
                    ...
                    {namhoctuongung.map((namHoc) =>
                        hockytuongung.map((hk) => {
                            ...
                            const filteredItems = resultArray.filter(
                                (item) => item.NamHoc === namHoc._id && item.HocKy === hk._id,
                            );
                            const DiemTL = DiemTichLuy.filter(
                                (item) => item.NamHoc === namHoc._id && item.HocKy === hk._id,
                            );
                            return (
                                <TableTest
                                    row={filteredItems} // Pass the filtered items
                                    NamHoc={namHoc}
                                    hocKy={hk}
                                    DiemTichLuy={DiemTL}
                                />
