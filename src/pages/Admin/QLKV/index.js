    ...
    const columns = [
        ...
        },
        {
            ...
            renderCell: (params) => (
                <div
                    style={{ display: 'flex', cursor: 'pointer', alignItems: 'center', justifyContent: 'center' }}
                    onClick={() => handleDRLClick(params.row)}
                >
                    <AutoStoriesRounded style={{ color: 'blue' }} />
                    <span title="Chỉnh sửa" style={{ marginLeft: '8px' }}>
                        Xem điểm rèn luyện
                    </span>
                </div>
            ...
            <div className={cx('wrapperr')}>
                <div className={cx('innerr')}>
                    <div className={cx('bgr-content')}>
                        ...
                        {onCancel ? <DeleteLopHoc onCancel={handleUpdateDiemCancel} LopHocId={selectedIDs} /> : ''}
