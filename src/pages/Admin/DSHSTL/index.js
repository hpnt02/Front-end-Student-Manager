...
import { MyDocument } from '../In/KiemTra';
...
function DSHSTL() {
 ...
 HSLH.forEach((hs,index) => {
  ...
  const handleExport = () => {
    ...
    const rows = resultArray.map(item => [item.id, item.MaHocSinh, item.TenHocSinh,item.GioiTinh ,item.NgaySinh, item.DiaChi]);
    ...
    return ( 
        <div className={cx('wrapperr')}>
         ...
         <div className={cx('export-data')}>
            <PDFDownloadLink document={<MyDocument hs={resultArray}/>} fileName="test">
             {({loading}) => (loading ? <Button primary>Loading Document ...</Button> :<Button primary>Download</Button>)}
         </PDFDownloadLink> 
