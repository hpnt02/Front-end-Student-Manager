...
function QLMH() {
  ...
  function handleFilter(event) {
   ...
   const newData = resultArray.filter(row => {
     ...
     return (
      tenmonhoc.includes(searchValue) ||
       maMonHoc.includes(searchValue)
     );
   });
  ...
  return (  
    ...
    <div className={cx('innerr')}>
           <div className={cx('bgr-content')}>
            ...
            <div className={cx('search')}><input type="text" onChange={handleFilter} placeholder="Nhập mã môn học hoặc tên môn học"/></div>               
               
             <div className={cx('button-create')}>
         
              <Button type="text" primary to={config.routesAdmin.tmmh}>Thêm mới môn học</Button>
              <Button type="text" disabled={selectedIDs.length === 0} primary onClick={handleClickCancel}>Xóa môn học</Button>    
              <Button type="text" primary to={config.routes.home}>Quay lại</Button>        
            </div>

       </div>
       {onCancel ?                
                  <DeleteMonHoc onCancel={handleUpdateDiemCancel} MonHocID={ selectedIDs}/>:""
                }
