...
import { DSHS, getKhoa, getLoiViPham, getMonHoc,getNGANH,GiaoVien, getHocKy, getNamHoc} from '~/redux/apiAdmin/apiAdmin';
            ...
            <div className={cx('bgr-content')}>
                 <h2 className={cx('title')}>CHI TIẾT LỖI VI PHẠM</h2>
                        <div className={cx('infor-student')}>
                            ...
                            <p>
                                <strong>Tên học sinh:</strong> {hocsinhtuongung? hocsinhtuongung.HoHS:""} {hocsinhtuongung? hocsinhtuongung.TenHS:""}
                            </p>
                            <p>
                    ...
                    <div className={cx('button-chucnang')}>
                      { user !== '3' ?
                      <>
                          <Button type="text" disabled={selectedIDs.length === 0} primary onClick={handleClickCancel}>Xóa lỗi vi phạm</Button>   
                          <Button primary to={config.routesAdmin.diemrenluyen}>Quay lại</Button>
                          </> 
                          :
                     <Button primary to={config.routes.drl}>Quay lại</Button>
                        }
                    </div>
                    {onCancel ?                
                  <DeleteLVP onCancel={handleUpdateDiemCancel} LoiViPhamId={ selectedIDs}/>:""
                }
          <DataGridDemo columns={columns} rows={resultArray} data={[]} onClick={handleIDChange} check={user === '3' ? true : false}/>
                </div>
                </div>
                </div>
     );
